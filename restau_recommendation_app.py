...
def recommend():
    ...
    if use_default:
        dish_file_with_path = "default_dish_list.txt"  
    else:
        dish_file_with_path = os.path.join(app.config['UPLOAD_FOLDER'], "uploaded_dish_list.txt")

    
    base_path="data"
    results = get_top_restaurants(cuisine, dish, base_path, dish_file_with_path)
    if not results:
        return render_template("recommendation_result.html", recommendations=[],
                                                            selected_dish=dish,
                                                            selected_cuisine=cuisine,
                               message=f"Could not find {cuisine} restaurants that serve {dish}")
    else:
